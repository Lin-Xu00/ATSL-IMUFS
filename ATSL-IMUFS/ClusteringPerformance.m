function [ACC_mean, NMI_mean] = ClusteringPerformance(X, Y, W, kmeans_iter, fea_rate)
% This function ranks and selects features based on their L2-norm scores
% computed from the feature selection matrices W, and evaluates clustering
% performance using k-means clustering algorithm on the selected features.
%
% Inputs:
%   X           - 1-by-v cell array, where each X{l} is a d_l-by-n data matrix for the l-th view
%   Y           - Ground-truth labels
%   W           - 1-by-v cell array of feature selection matrices
%   kmeans_iter - Number of k-means runs for statistical reliability
%   fea_rate    - feature selection ratios (values between 0 and 1)
%
% Outputs:
%   ACC_mean    - Mean clustering accuracy (ACC) across runs
%   NMI_mean    - Mean normalized mutual information (NMI) across runs
...
% Stack feature matrices from all views vertically to form a unified feature matrix
X_hat_Multi = [];
for l = 1:v
    X_hat_Multi = [X_hat_Multi; X{l}];
end

% Compute L2-norm scores for each feature in all views
score = cellfun(@(X) sqrt(sum(X.^2, 2)), W, 'UniformOutput', false);
score = vertcat(score{:});  % Concatenate scores from all views into a single vector

% Rank all features based on their L2-norm scores in descending order
[~, mrrfs_f_idx] = sort(score, 'descend');

% Reorder the rows of X_hat_Multi according to feature importance
Fea_fs = X_hat_Multi(mrrfs_f_idx, :); 

% Initialize containers for clustering results
ACC_fs = zeros(kmeans_iter, length(fea_rate));  
NMI_fs = zeros(kmeans_iter, length(fea_rate));

for i = 1:length(fea_rate)

    % Total number of features across all views
    d_fea = size(X_hat_Multi, 1);  

    % Number of selected features
    fea_num = ceil(fea_rate(i) * d_fea);  
    ...
    REPlic = 20;        % Number of replicates
    class_num = c;      % Number of clusters

    % Perform k-means clustering multiple times to compute averaged results
    parfor ii = 1:kmeans_iter  
        idx = kmeans(fea_fs', class_num, ...
                     'maxiter', MAXiter, ...
                     'replicates', REPlic, ...
                     'EmptyAction', 'singleton');

        % Evaluate clustering performance using ACC and NMI
        result = ClusteringMeasure(Y, idx);
        ACC_fs(ii, i) = result(1, 1);  
        NMI_fs(ii, i) = result(1, 2);  
    end
end

% Compute the mean clustering performance over all k-means runs
ACC_mean = mean(ACC_fs, 1) * 100;  
NMI_mean = mean(NMI_fs, 1) * 100;
end
